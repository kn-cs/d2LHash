...
def getKeyRandom(k):
	...
	key = []
	f = open("d2LHash1271_d4_verify_keyfile.txt", "w")
	for i in range(k): 
...
def d2LHash(t, d, msgLen, keyFldElt, msgEltsLst):
	...
	for i in range(0,frakm):
		...
		for j in range(0,delta):
			tmpMsgLst.append(msgEltsLst[i*delta+j])
		Xlist.append(ComputeBRW(t,keyFldElt,tmpMsgLst))
	X = Horner(gamma, Xlist)
	tmpMsgLst = [X]
	for j in range(delta*frakm,m):
		tmpMsgLst.append(msgEltsLst[j])
...
def genTestCases():
	...
	t = 4;
	d = 4;
	f1 = open("d2LHash1271_d4_verify_test_cases.txt", "w")
	f2 = open("d2LHash1271_d4_verify_s_outputs.txt", "w")	
	start = 1
	stop = 2040
	step = 1
	for msgLen in range(start,stop,step):	
		for i in range(1,2):
			...
			f1.write("%d"%msgLen)
			f1.write("\n")
			digest = d2LHash(t, d, msgLen, keyFldElt, msgEltsLst)
