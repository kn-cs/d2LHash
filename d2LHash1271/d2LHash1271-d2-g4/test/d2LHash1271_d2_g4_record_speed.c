...
int main() {
	...
	FILE *fpin,*fpout;
    	...
    	// #define SAGE
    	#ifdef SAGE    	
    	c = system("cd ./test_files; sage d2LHash1271_d2_speed_record_test_cases.sage; cd ..");
    	remove("./test_files/d2LHash1271_d2_speed_record_test_cases.sage.py");    	
    	#endif     	
    	
	format_key(k,KEY_LENGTH,"./test_files/d2LHash1271_d2_speed_record_test_cases_keyfile.txt");
	q = (uint64 *)k; kp[0] = q[0]; kp[1] = q[1];	
    	
   	fpin = fopen("./test_files/d2LHash1271_d2_speed_record_test_cases.txt", "r");
    	if (fpin == NULL) {
	...
	fpout = fopen("./test_files/d2LHash1271_d2_g4_speed_records.txt","w");
